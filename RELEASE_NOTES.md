...
- 启用 / 禁用个性化设置开关。
- **「关于你」**：用户昵称、用户职业、用户介绍三项汇总为一个小节，保存后 Agent 在回复中使用称呼并结合你的背景。
- **「特质」小节**：标题和列表、表情符号两个微调项汇总为一个小节。
...
- **「关于你」进入提示词与工具**：新增 `occupation`（用户职业）与 `bio`（用户介绍）配置项，编译进 system prompt（Identity 层）；`set_persona` 工具新增对应参数；`/soul show` 输出新增「职业」「介绍」两行。
